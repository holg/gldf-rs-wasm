...
impl Component for App {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        html! {
            <div id="wrapper">
                <p id="title">{ "Upload GLDF Datei" }</p>
...
fn parse_file_for_gldf(file: &FileDetails) -> FileBufGldf{
    return GldfProduct::load_gldf_from_buf_all(file.data.clone()).unwrap();
}
...
impl App {
    ...
    fn view_file(file: &FileDetails) -> Html {
        fn view_gldf_file(buf_file: &BufFile) -> Html {
            html! {
                <div id="buf_file">{ format!("{}", buf_file.name.clone().unwrap_or("".to_string())) }
